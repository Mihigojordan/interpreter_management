...
import {
  Search, ChevronDown, Eye, ChevronLeft, ChevronRight,
  AlertCircle, RefreshCw, Filter, Grid3X3, List, Settings, Minimize2,CheckCircle, XCircle
} from 'lucide-react';
...
const InterpreterRequestDashboard = () => {
  const [requests, setRequests] = useState([]);
  const [allRequests, setAllRequests] = useState([]);
  const {user} = useInterpreterAuth()
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const reqs = await interpretationRequestService.getAllRequestsByInterpreters(user.id);
      setAllRequests(Array.isArray(reqs) ? reqs : []);
      setError(null);
    } catch (err) {
      setError(err.message || 'Failed to load your interpretation requests');
      setAllRequests([]);
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-gray-100 font-sans">
        ...
        <div className="mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-xl font-semibold text-gray-900">My Interpretation Requests</h1>
                <p className="text-sm text-gray-500">View your assigned interpretation requests</p>
              </div>
            </div>
            <div className="flex items-center space-x-3">
              <motion.button
      ...
      <div className="mx-auto px-4 py-6 space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { title: 'Total Assigned Requests', count: totalRequests, color: 'primary-600', icon: Settings },
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-lg shadow border border-gray-100 p-8 text-center">
            <p className="text-lg font-semibold text-gray-900">
              {searchTerm ? 'No Requests Found' : 'No Assigned Requests'}
            </p>
            <p className="text-sm text-gray-500 mt-1">
              {searchTerm ? 'Try adjusting your search criteria.' : 'No interpretation requests assigned to you.'}
            </p>
          </div>
        ) : (
          <div>
...
export default InterpreterRequestDashboard;